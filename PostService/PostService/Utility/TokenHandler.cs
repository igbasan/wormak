            ...
            if (!string.IsNullOrWhiteSpace(authToken))
            {
                if (isUserToken)
                {
                    ...
                    if (profile != null)
                    {
                        ...
                        identity.AddClaim(new Claim("http://schemas.xmlsoap.org/ws/2005/05/identity/claims/interests", JsonSerializer.Serialize(profile.Interests ?? new List<Models.Interest>())));
